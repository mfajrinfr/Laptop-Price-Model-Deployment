...
# Terapkan permintaan pindah halaman (dari tombol goto()) SEBELUM widget
# dropdown di bawah ini dibuat. Streamlit melarang mengubah st.session_state
# untuk key milik sebuah widget SETELAH widget itu di-render pada run yang
# sama, makanya goto() tidak langsung menulis ke "menu_page", tapi lewat
# key perantara "requested_page" yang dipindahkan ke sini terlebih dahulu.
if "requested_page" in st.session_state:
    st.session_state.menu_page = st.session_state.pop("requested_page")
...
def load_artifacts():
    ...
    company_map = joblib.load("company_segment_mapping.pkl")
    cputier_map = joblib.load("cpu_tier_mapping.pkl")
    gpubrand_map = joblib.load("gpu_brand_mapping.pkl")
    typename_encoder = joblib.load("typename_onehot_encoder.pkl")
    scaler = joblib.load("standard_scaler.pkl")
    return model, company_map, cputier_map, gpubrand_map, typename_encoder, scaler


def predict_price(raw_input: dict, model, company_map, cputier_map, gpubrand_map, typename_encoder, scaler) -> float:
    """Terima 1 baris input mentah (dict), kembalikan prediksi harga (EUR)."""
    df = pd.DataFrame([raw_input])

    # Company, CpuTier, GpuBrand: ordinal mapping (bukan one-hot) sesuai hasil
    # agregasi/segmentasi yang dilakukan saat training.
    df["Company"] = df["Company"].map(company_map)
    df["CpuTier"] = df["CpuTier"].map(cputier_map)
    df["GpuBrand"] = df["GpuBrand"].map(gpubrand_map)

    # TypeName: tetap One-Hot Encoding
    encoded = typename_encoder.transform(df[["TypeName"]])
    encoded_df = pd.DataFrame(
        encoded, columns=typename_encoder.get_feature_names_out(["TypeName"]), index=df.index
    )
    X = pd.concat([df.drop(columns=["TypeName"]), encoded_df], axis=1)
...
"""
    ...
    st.caption(
        ...
        "Kaggle 'muhammetvarl/laptop-price') dan hanya untuk keperluan "
        "demonstrasi. Hasil prediksi adalah estimasi statistik, bukan harga pasti."
    )
...
- **GPU Brand** — merek kartu grafis (Intel, Nvidia, AMD, atau ARM)
...
"""
    ...
    st.info("Dengan spesifikasi di atas, model memberikan estimasi harga sekitar **€ 971**.")
...
# ---------------------------------------------------------------------------
def show_prediction():
    model, company_map, cputier_map, gpubrand_map, typename_encoder, scaler = load_artifacts()

    company_options = list(company_map.keys())
    cputier_options = list(cputier_map.keys())
    gpubrand_options = list(gpubrand_map.keys())
    typename_options = list(typename_encoder.categories_[0])
    ...
    st.write(
        ...
        "Lihat halaman **📖 Panduan Pemakaian** di menu sidebar."
    )
    ...
    st.markdown(
        "**💾 Storage**  \n"
        "Isi sesuai fisik laptop. Kalau laptop punya 2 slot storage yang beda "
        "jenis atau sama-sama SSD/HDD, isi Layer 1 & Layer 2 masing-masing — "
        "totalnya akan dihitung otomatis."
    )
    ...
    if submitted:
        ...
        try:
            price_eur = predict_price(
                raw_input, model, company_map, cputier_map, gpubrand_map, typename_encoder, scaler
            )
